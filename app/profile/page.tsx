  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f3f7fa" }}>
      <header style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "14px 24px", display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <Link href="/camfolder" style={{ marginLeft: "auto", fontSize: 13, fontWeight: 600, color: "#4a7a9b", textDecoration: "none" }}>
          ← Back to Projects
        </Link>
      </header>
      ...
      <main style={{ maxWidth: 520, margin: "0 auto", padding: "32px 16px 80px" }}>
        ...
        <ProfileForm currentName={user.user_metadata?.display_name ?? ""} email={user.email ?? ""} googleReviewUrl={user.user_metadata?.google_review_url ?? ""} businessEmail={user.user_metadata?.business_email ?? ""} />
      </main>
    </div>
  );
}
